...
| `.mcp.json` | Claude Code / Codex MCP connection — **stdio by default** through `npx -y @dinglebear/yarr@2.2.2 mcp`, one `YARR_<NAME>_*` env var per `userConfig` field. No committed platform binary or separately-run server is required. |
...
`npx`, `args` starts with `["-y", "@dinglebear/yarr@2.2.2", "mcp"]`, and `env` maps every
