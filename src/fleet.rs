//! Host-owned fleet invocation and dispatch types.
//!
//! Fleet selectors are resolved against configured identities before any
//! authorization or upstream work begins, and the resolved set is frozen for
//! one invocation — there is no script-wide plan and no replay. Fleet is
//! deliberately not an action in the MCP registry: Code Mode reaches it
//! through the private `__yarrFleetMap` / `__yarrFleetStatus` bridge.
...
/// The frozen leaf set materialized for one fleet invocation. Leaves are exact
/// configured identities bound to their immutable parsed action.
#[derive(Debug, Clone)]
...
impl PlannedFleetLeaf {
    ...
    pub(crate) fn action_service(&self) -> &str {
        match &self.action {
            ...
            | YarrAction::SnippetDelete { .. } => unreachable!(),
        }
    }
}

/// Transport-neutral label for one concrete fleet leaf. Used for batch
/// destructive authorization (the prompt lists exactly these) and audit rows.
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct FleetLeafLabel {
    pub service: String,
    pub action: String,
}
...
/// Parse the private bridge params emitted by the `fleet` preamble global.
/// Strict: unknown fields fail closed so a stale/newer script can never smuggle
/// extra semantics past the bridge.
...
/// Truncation metadata replacing an oversized leaf value: what it was, how many
/// items it held, and its observed encoded size — never a silent cut.
