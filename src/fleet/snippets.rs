//! Canonical read-only Code Mode snippets shipped with the binary.
//!
//! Builtins are never persisted: `snippet_list` surfaces them alongside stored
//! snippets, `snippet_run` resolves them by name, and the store refuses to
//! save or delete a snippet whose name is reserved here (so a builtin can never
//! be shadowed or overwritten once protection is in place).
//!
//! All sources are read-only observability fan-outs (`fleet.map` over curated
//! read actions or `fleet.status`), so they never require destructive
//! confirmation.
...
const BUILTINS: [BuiltinSnippet; 4] = [
    ...
    BuiltinSnippet {
        ...
        source: "async () => fleet.map(fleet.all('tracearr'), 'trace_streams')",
    },
];
