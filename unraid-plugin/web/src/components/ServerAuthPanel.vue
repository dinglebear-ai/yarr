...
<template>
  <section class="yarr-panel" aria-labelledby="server-heading">
    ...
    <div class="yarr-auth-section">
      <SecretField v-if="plugin.authMode === 'BEARER'" name="bearer-token" label="Bearer token" :configured="bearerConfigured" :intent="auth.bearerToken.kind" :disabled="disabled" generate @update="authSecret('bearerToken', $event)" />
