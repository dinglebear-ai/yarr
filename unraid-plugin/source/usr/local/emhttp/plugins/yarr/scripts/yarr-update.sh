...
yarr_update_bootstrap_paths() {
    local physical_path=$1 plugin_root release_helper
    if yarr_update_is_installed_path "$physical_path"; then
        printf '%s\n' /usr/local/emhttp/plugins/yarr/scripts/yarr-common.sh /etc/rc.d/rc.yarr /usr/local/emhttp/plugins/yarr/scripts/yarr-update-release.sh
        return 0
    fi
    plugin_root=${YARR_PLUGIN_ROOT:-/usr/local/emhttp/plugins/yarr}
    release_helper=${YARR_UPDATE_RELEASE_HELPER:-"${plugin_root}/scripts/yarr-update-release.sh"}
    printf '%s\n' "${plugin_root}/scripts/yarr-common.sh" "${YARR_RC_YARR:-/etc/rc.d/rc.yarr}" "$release_helper"
}
...
YARR_UPDATE_RELEASE_HELPER=${yarr_update_bootstrap[2]}
...
# shellcheck source=/usr/local/emhttp/plugins/yarr/scripts/yarr-update-release.sh
source "$YARR_UPDATE_RELEASE_HELPER" || {
    yarr_update_error 'cannot read release helper'
    exit 1
}
