...
def check_ci(workflow: dict[str, Any]) -> None:
    ...
    require_line(api, "cd unraid-plugin/api && npm ci && npm audit --audit-level=high && npm test && npm run typecheck && npm run build", "CI API gate is incomplete")
    require_line(web, "cd unraid-plugin/web && npm ci && npm audit --audit-level=high && npm test && npm run typecheck && npm run build", "CI web gate is incomplete")
...
def check_release(workflow: dict[str, Any]) -> None:
    ...
    release_api = named_step(build, "Test and build API extension", "release.build")
    release_web = named_step(build, "Test and build settings and dashboard elements", "release.build")
    require_line(release_api, "cd unraid-plugin/api && npm ci && npm audit --audit-level=high && npm test && npm run typecheck && npm run build", "release API gate is incomplete")
    require_line(release_web, "cd unraid-plugin/web && npm ci && npm audit --audit-level=high && npm test && npm run typecheck && npm run build", "release web gate is incomplete")
